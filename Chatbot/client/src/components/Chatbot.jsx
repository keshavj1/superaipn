...
import { MessageSquare, Send, Sparkles, User, Bot, Loader2, Minimize2, ChevronDown } from 'lucide-react';
import superAipLogo from '../../../../src/assets/super_aip_logo.png';
...
const Chatbot = () => {
  ...
  const [messages, setMessages] = useState([
    { text: "Hello! I'm Super AIP. How can I assist you today? ", sender: 'bot', time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) }
  ]);
  ...
  return (
    <div className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-[9999] flex flex-col items-end font-sans">
      {/* Main Chat Window */}
      <div
        className={`mb-4 flex flex-col overflow-hidden rounded-[2rem] border border-white/20 bg-white shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1),0_0_20px_rgba(16,185,129,0.1)] transition-all duration-500 origin-bottom-right ${isOpen ? 'h-[600px] sm:h-[650px] w-[calc(100vw-2rem)] sm:w-[400px] max-h-[85vh] opacity-100 scale-100' : 'h-0 w-0 opacity-0 scale-95 pointer-events-none'
          }`}
      >
        {/* Header */}
        <div className="relative flex items-center justify-between px-6 py-5 overflow-hidden bg-gradient-to-r from-emerald-600 to-teal-500 text-white shrink-0" style={{
          padding: "20px",
          borderRadius: "20px",
          background: '#6f63f2',
        }}>
          ...
          <div className="relative flex items-center gap-3">
            <div className="relative flex h-11 w-11 items-center justify-center overflow-hidden">
              <img src={superAipLogo} alt="Super AIP" className="h-8 w-8 object-contain" />
        ...
        {/* Messages Area */}
        <div className="flex-1 space-y-5 overflow-y-auto p-6 scrollbar-thin scrollbar-thumb-emerald-200 scrollbar-track-transparent bg-slate-50" style={{
          padding: '10px 10px',
        }}>
            ...
            >
              <div className={`flex gap-2 max-w-[85%] ${msg.sender === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                ...
                <div className="shrink-0 flex items-end">
                  <div className="flex h-7 w-7 items-center justify-center rounded-full shadow-sm text-white bg-[#7468f2]">
                ...
                <div className={`flex flex-col ${msg.sender === 'user' ? 'items-end' : 'items-start'}`}>
                  <div className={`relative px-4 py-3 text-[14px] leading-relaxed shadow-sm text-white ${msg.sender === 'bot'
                    ? 'rounded-[1.25rem] rounded-bl-sm bg-[#7266f2] border border-white/10'
                    : 'rounded-[1.25rem] rounded-br-sm bg-[#7266f2]'
                    }`}>
                    {msg.text}
                  </div>
        ...
        <div className="p-4 bg-white border-t border-slate-100 shrink-0">
          ...
          <div className="relative flex items-center rounded-2xl bg-slate-50 overflow-hidden border border-slate-200 focus-within:border-emerald-500/50 focus-within:ring-4 focus-within:ring-emerald-500/10 transition-all shadow-sm focus-within:bg-white">
            ...
            <div className="pr-2">
              <button
                className={`flex h-10 w-10 items-center justify-center rounded-xl transition-all duration-300 ${input.trim()
          ...
          <div className="mt-3 text-center">
            <span className="text-[10px] text-slate-400 font-medium">Powered by Super AIP Technology</span>
          </div>
        </div>
      </div>
      ...
      <button
        className={`group relative flex h-14 w-14 sm:h-16 sm:w-16 items-center justify-center transition-all duration-500 shadow-2xl ${isOpen
